...
export class Module {
   ...
   * ```
   */
  public addProvider(provider: IProvider): InjectionToken {
  ...
  private addFactoryProvider(provider: FactoryProvider): void {
    this._providers.set(
      ...
      new InstanceWrapper({
        ...
        scope: provider.scope ?? IScope.DEFAULT,
        host: this,
      })
    );
  }
   ...
   *
   * Falls back to `IScope.DEFAULT` (singleton) if no scope options
   * are defined on the class.
   *
   * @param type - The class to read scope metadata from
   * @returns The provider scope
   */
  private getClassScope(type: Type<any>): IScope {
    const options = getMetadata<{ scope?: IScope }>(SCOPE_OPTIONS_METADATA, type);
    return options?.scope ?? IScope.DEFAULT;
  }
