...
export class Injector {
  ...
  private async resolveClass<T>(wrapper: InstanceWrapper<T>, moduleRef: Module): Promise<T> {
    ...
    // Resolve each dependency SEQUENTIALLY. Parallel resolution with a
    // shared `resolutionStack` produces false circular-dependency errors
    // when sibling deps share a transitive provider — e.g. constructor
    // param N+1 starts resolving while N is still mid-stack, both step
    // into the same provider, and the second one sees the first's mark
    // and aborts. Nest's injector also serializes this for the same reason.
    ...
    for (let index = 0; index < deps.length; index++) {
      ...
      if (dep === undefined || dep === null || dep === Object) {
        if (optionalIndices.includes(index)) {
      ...
      try {
        resolvedDeps.push(await this.resolveDependency(dep, moduleRef));
      } catch (err: Error | any) {
   ...
   *
   * @param value - Candidate inject array entry
   * @returns Whether the entry is the optional-wrapper form
   */
  private isOptionalFactoryDependency(
    value: unknown
  ): value is { token: InjectionToken; optional: boolean } {
  ...
  private async resolveFactory<T>(wrapper: InstanceWrapper<T>, moduleRef: Module): Promise<T> {
    ...
    const injectEntries = wrapper.inject ?? [];
    ...
    // ── 1. Normalize inject entries into bare tokens, tracking which
    //    indices are optional (mirrors Nest's
    //    `getFactoryProviderDependencies` shape).
    const optionalIndices: number[] = [];
    const tokens: InjectionToken[] = injectEntries.map((entry, index) => {
      if (this.isOptionalFactoryDependency(entry)) {
        if (entry.optional) optionalIndices.push(index);
        return entry.token;
      }
      return entry as InjectionToken;
    });

    // ── 2. Resolve each token; honour optional indices on failure.
    //
    // Sequential resolution mirrors NestJS — parallel `Promise.all`
    // produces false circular-dependency errors when sibling factory
    // deps share a transitive provider.
    const resolvedDeps: any[] = [];
    for (let index = 0; index < tokens.length; index++) {
      const token = tokens[index]!;
      try {
        resolvedDeps.push(await this.resolveDependency(token, resolveContext));
      } catch (err: Error | any) {
        if (optionalIndices.includes(index)) {
          resolvedDeps.push(undefined);
          continue;
    ...
    // ── 3. Call the factory and handle async results.
  ...
  private async resolveProperties<T>(instance: T, type: Type<T>, moduleRef: Module): Promise<void> {
    ...
    const resolvedProperties = (properties ?? []) as Array<{
      key: string | symbol;
      type: InjectionToken;
    }>;
    const resolvedOptionalKeys = (optionalKeys ?? []) as Array<string | symbol>;
