...
export function forwardRef<T = any>(fn: () => T): ForwardReference<T> {
  // The factory stores the thunk under `forwardRef`. The type field is
  // declared as `T` to match Nest's `ForwardReference<T>` shape (see
  // forward-reference.interface.ts), so we cast through `unknown`
  // here. Consumers go through `resolveForwardRef`, which casts back.
  return { forwardRef: fn as unknown as T };
}
