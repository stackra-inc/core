...
export class ApplicationContext {
  ...
  /** Current environment name. Reads from APP_ENV or NODE_ENV. Defaults to "production". */
  public get environment(): string {
    return (env('APP_ENV') ?? env('NODE_ENV') ?? 'production').toLowerCase();
  }
   ...
   * @returns A scoped `NestApplicationContext` for the selected module
   *
   ...
   * ```
   */
  public select<T>(module: Type<T> | DynamicModule): ScopedApplicationContext {
    this.assertInitialized();
  ...
  public enableShutdownHooks(): this {
    ...
    if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
      // Browser environment — use beforeunload (not available in React Native)
    ...
    // React Native: neither window.addEventListener nor process.on exist —
    // shutdown hooks are not applicable (app lifecycle managed by OS)
  ...
  /**
   * Extract the module class from a `Type` or `DynamicModule`.
   *
   * @param module - The module class or dynamic module object
   * @returns The module class constructor
   */
  private extractModuleClass(module: Type<any> | DynamicModule): Type<any> {
 ...
 * `NestApplicationContext` interface but scoped to a single module.
 *
 * @internal
 */
/**
 * @internal
 */
export class ScopedApplicationContext {
  /**
   ...
   */
  public select(): ScopedApplicationContext {
    return this;
  }
