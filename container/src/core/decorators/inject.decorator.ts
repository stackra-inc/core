...
export function Inject(
  ...
  return (target: object, key: string | symbol | undefined, index?: number) => {
    ...
    if (resolvedToken && typeof resolvedToken === 'object' && 'forwardRef' in resolvedToken) {
      // Same dance as in scanner.service.ts: the interface types
      // `forwardRef` as `T` for Nest compat; runtime stores a thunk.
      const thunk = (resolvedToken as { forwardRef: unknown }).forwardRef;
      resolvedToken = (
        typeof thunk === 'function' ? (thunk as () => unknown)() : thunk
      ) as typeof resolvedToken;
    }
