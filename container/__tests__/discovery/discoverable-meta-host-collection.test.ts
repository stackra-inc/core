...
import { Module as ModuleRef } from '@/core/container/module';
...
describe('DiscoverableMetaHostCollection', () => {
  ...
  describe('addClassMetaHostLink', () => {
    it('registers a class -> key mapping', () => {
      DiscoverableMetaHostCollection.addClassMetaHostLink(ServiceA, '@discoverable:test-key');
    ...
    it('overwrites an existing link with the latest key (matches NestJS behavior)', () => {
      DiscoverableMetaHostCollection.addClassMetaHostLink(ServiceA, '@discoverable:first');
      DiscoverableMetaHostCollection.addClassMetaHostLink(ServiceA, '@discoverable:second');
      expect(DiscoverableMetaHostCollection.metaHostLinks.get(ServiceA)).toBe(
        '@discoverable:second'
      );
    });
  ...
  describe('inspectProvider', () => {
    it('indexes class providers whose metatype is registered', () => {
      DiscoverableMetaHostCollection.addClassMetaHostLink(ServiceA, '@discoverable:k1');
    ...
    it('dedupes the same wrapper across multiple calls', () => {
      DiscoverableMetaHostCollection.addClassMetaHostLink(ServiceA, '@discoverable:dup');
    ...
    it('isolates index entries per ModuleContainer', () => {
      DiscoverableMetaHostCollection.addClassMetaHostLink(ServiceA, '@discoverable:iso');
      ...
      expect(
        DiscoverableMetaHostCollection.getProvidersByMetaKey(containerA, '@discoverable:iso').size
      ).toBe(1);
      expect(
        DiscoverableMetaHostCollection.getProvidersByMetaKey(containerB, '@discoverable:iso').size
      ).toBe(0);
    });

    it('falls back to instance.constructor for useValue providers', () => {
      DiscoverableMetaHostCollection.addClassMetaHostLink(ServiceA, '@discoverable:value');
    ...
    it('does not index useFactory providers at scan-time', () => {
      DiscoverableMetaHostCollection.addClassMetaHostLink(ServiceA, '@discoverable:factory');
  ...
  describe('getProvidersByMetaKey', () => {
    it('returns an empty Set when nothing is registered', () => {
      const container = new ModuleContainer();
      expect(
        DiscoverableMetaHostCollection.getProvidersByMetaKey(container, '@discoverable:missing')
          .size
      ).toBe(0);
    });

    it('returns multiple wrappers for the same key in registration order', () => {
      DiscoverableMetaHostCollection.addClassMetaHostLink(ServiceA, '@discoverable:multi');
      DiscoverableMetaHostCollection.addClassMetaHostLink(ServiceB, '@discoverable:multi');
      ...
      expect(
        Array.from(
          DiscoverableMetaHostCollection.getProvidersByMetaKey(container, '@discoverable:multi')
        )
      ).toEqual([a, b]);
    });
  });
