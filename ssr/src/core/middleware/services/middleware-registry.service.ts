 ...
 *
 *   Holds all middleware and groups. Consumers resolve the registry
 *   through the `MIDDLEWARE_REGISTRY` DI token (declared in
 *   `@stackra/contracts`).
 *
 *   The registry is purely imperative — it is populated by
 *   `MiddlewareDiscovery` (scanning `@Middleware()` classes) and by any
 *   explicit `register()` / `registerGroup()` calls made by the module's
 *   wire-time seed factory. It holds no config and implements no
 *   lifecycle hook.
 */
 ...
 *
 * Purely imperative — populated by `MiddlewareDiscovery` (scanning
 * `@Middleware()` classes) and by any explicit `register()` /
 * `registerGroup()` calls. It holds no config and implements no
 * lifecycle hook: registration is driven by discovery, not by a
 * middleware config file.
 *
 * Duplicate name registrations replace the earlier entry and emit a
 * warning via the optional logger.
 */
@Injectable()
