...
export class Collection<T = any> {
  ...
  splice(start: number, length?: number, ...items: T[]): Collection<T> {
    ...
    return new Collection(
      this.collection.splice(start, actualLength, ...itemsArray).all(),
    );
  }
  ...
  where(key: keyof T, operatorOrValue: any, value?: any): Collection<T> {
    if (value === undefined) {
      return new Collection(
        this.collection.where(key as any, operatorOrValue).all(),
      );
    }
    return new Collection(
      this.collection.where(key as any, operatorOrValue, value).all(),
    );
  }
  ...
  zip<U>(...arrays: U[][]): Collection<any[]> {
    ...
    return new Collection(
      (this.collection as any).zip(...zipArgs).all() as any[],
    );
  }
